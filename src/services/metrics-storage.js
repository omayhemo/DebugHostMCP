...
class MetricsStorage {
    ...
    try {
      ...
      for (const [containerId, data] of this.highResData) {
        ...
        if (!Array.isArray(data)) {
          console.warn(`High-res data for container ${containerId} is not an array, removing`);
      ...
      for (const [containerId, aggregated] of this.aggregatedData) {
        ...
        for (const [interval, data] of Object.entries(aggregated)) {
          // Ensure data is an array
          if (!Array.isArray(data)) {
            console.warn(`Aggregated data for container ${containerId} interval ${interval} is not an array`);
            aggregated[interval] = [];
            continue;
          }
  ...
  aggregateDataForInterval(highResData, intervalMs, existingAggregated) {
    // Ensure highResData is an array
    if (!Array.isArray(highResData)) {
      console.warn('High-res data is not an array, skipping aggregation');
      return 0;
    }
