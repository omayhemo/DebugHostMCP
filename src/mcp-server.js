...
async function initializeMetricsInfrastructure() {
  try {
    ...
    // Register Docker container routes
    const dockerRoutes = require('./routes/docker-routes');
    app.use('/api', dockerRoutes);
    console.log('Docker container routes registered');
    
    // Register servers adapter routes (for dashboard compatibility)
    const serversAdapter = require('./routes/servers-adapter');
    app.use('/api', serversAdapter);
    console.log('Servers adapter routes registered');
