...
3. **Create new session note** with timestamp: `{{SESSION_NOTES_PATH}}/YYYY-MM-DD-HH-mm-ss-AP Orchestrator-Agent-Activation.md`
...
- **ENFORCE** standards defined in `{{AP_ROOT}}/config/document-registry.json`

### Required Document Patterns
- **Epics**: `EPIC-{id}-{title}.md` in `/project_docs/planning/epics/`
- **Stories**: `STORY-{id}-{title}.md` in `/project_docs/planning/planning-stories/`
...
**See**: `{{AP_PERSONAS}}/DOCUMENT_COMPLIANCE_RULES.md` for complete standards
