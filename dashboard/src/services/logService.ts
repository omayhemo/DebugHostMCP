...
class LogService {
  ...
  private connectEventSource(): void {
    ...
    try {
      ...
      const baseURL = apiService['client'].defaults.baseURL || 'http://127.0.0.1:2603';
  ...
  private createWebSocketConnection(options: LogStreamOptions): WebSocket {
    const baseURL = apiService['client'].defaults.baseURL?.replace(/^http/, 'ws') || 'ws://127.0.0.1:2603';
    const token = localStorage.getItem('token');
